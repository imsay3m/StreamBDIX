...
function getEnabledSources() {
    try {
        ...
        console.log("[StreamBDIX] Enabled sources:", enabled);
        return enabled.map((key) => allSources[key]).filter(Boolean);
    } catch {
        console.log("[StreamBDIX] Using all sources (fallback)");
...
// Timeout wrapper for source requests
...
builder.defineStreamHandler(async ({ type, id }) => {
    ...
    if (!meta || !meta.name) {
        // Cinemeta doesn't have full metadata for this item
        // This typically happens for very new releases
        return { streams: [] };
    }
    ...
    console.log(
        `[StreamBDIX] Fetching streams for ${meta.name} from ${relevantSources.length} sources:`,
        relevantSources.map((s) => s.name),
    );

    const streamPromises = relevantSources.map((source) =>
        withTimeout(source.getStreams(type, meta, season, episode), 8000).catch(
            (err) => {
                console.log(
                    `[StreamBDIX] Source ${source.name} failed:`,
                    err.message,
                );
                return [];
            },
    ...
    const results = await withTimeout(Promise.all(streamPromises), 15000).catch(
        (err) => {
            console.log(
                `[StreamBDIX] Overall fetch timeout (15s) - returning partial results`,
            );
            return [];
        },
    );
    ...
    console.log(`[StreamBDIX] Returning ${streams.length} streams`);
    return { streams };
});
